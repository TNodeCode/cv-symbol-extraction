...
class RealSequencesDataset(Dataset):
    def __init__(self, filename, vocab_in, vocab_out, max_length, batch_size, sos_idx=0, eos_idx=1, pad_idx=3, additional_eos=False, device="cpu", use_random=True) -> None:
        ...
        for i, f in enumerate(formulas):
            ...
            if len(parsed_formula) <= self.max_length-1 and self.boxes[i].shape[0] <= self.max_length-1:
                ...
                # target_seq = np.insert(target_seq, 0, 0)
                target_seq = np.append(target_seq, 1)
                # target_len = len(target_seq)
                # if target_len in sorted_formula.keys():
                #     sorted_formula[target_len].append(target_seq)
                # else:
                #     sorted_formula[target_len] = [target_seq]
                ...
                coords = self.pad_coordinates(coords, self.max_length)
    ...
    def __getitem__(self, index: int):
        ...
        use_random = (self.use_random) and (random.randint(0, 1) > 0.5)
        if use_random:
