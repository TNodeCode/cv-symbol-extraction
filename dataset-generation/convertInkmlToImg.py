        ...
        for annotation in annotations:
            try:
                if annotation.attrib['type'] == 'truth':
                    latex_code = annotation.text
                    break
            except KeyError:
                if annotation.attrib.get("typx") == 'truth':
                    latex_code = annotation.text
                    break
        # remove all whitespace from latex_code
        latex_code = latex_code.replace(" ", "")
        # if there is no $ at start or end of latex code, add it
        if latex_code[0] != "$":
...
def convert_to_imgs_2014(traces_data, box_axis_size, lg_dict):
    """
    ...
    for index, traces_all in enumerate(traces_data):
        ...
        # Centering: Shifts the pattern so that it is centered in the box
        centered_trace = center_pattern(scaled_trace, max_x=trace_width * scale_factor,
                                        max_y=trace_height * scale_factor, box_axis_size=box_axis_size - 1)  #

        trace_index[index] = centered_trace

        # Draw pattern on the background
        pattern_drawn = draw_pattern(centered_trace, pattern_drawn, box_axis_size=box_axis_size)

    label_lines = []
    for object in lg_dict.values():
        for symbol, trac in object.items():
            trac.sort()
            flag = True
            for i in trac:
                if flag:
                    final_array = np.array(trace_index[i])
                    flag = False
                else:
                    final_array = np.vstack((final_array, np.array(trace_index[i])))

        label_lines.append(create_bounding_box(final_array, box_axis_size, symbol))
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        ...
        lg_folder = "Test2013LG" if len(sys.argv) < 5 else sys.argv[4]
        ...
        for idx, file in enumerate(FILES):
            ...
            # if lg_file starts with "2014_"
            if lg_file.split(os.sep)[-1].split(".")[0].split("_")[0] == "2014":
                trace_counter = 0
                with open(lg_file, 'r') as f:
                    ...
                    for file_line in lines:
                        ...
                        if file_line.startswith("# Object: "):
                            flag = True
                            obj = file_line  # .split("Object: ")[1][:-1] DONT SPLIT IT DUPLICATES KEYS SOMETIMES
                            lg_node_dict[obj] = {}

                        if file_line.startswith("N"):
                            trace_counter += 1
                            Node = file_line.split(", ")
                            index = int(Node[1])
                            symbol = Node[2]
                            if flag:
                                lg_node_dict[obj][symbol] = [index]
                                flag = False
                            else:
                                lg_node_dict[obj][symbol].append(index)
                f.close()

                traces, latex_code = parse_inkml(img_path + os.sep + img_name)
                selected_tr = get_traces_data(traces)

                if len(selected_tr) != trace_counter:
                    print("In LG-File " + str(lg_file) + " are not all traces.")
                    continue

                im, label_lines = convert_to_imgs_2014(selected_tr, dim, lg_node_dict)
            else:
                with open(lg_file, 'r') as f:
                    # Read all lines in the file
                    lines = f.readlines()
